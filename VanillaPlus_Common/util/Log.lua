...
local function Serialize(data)
	...
	elseif(dataType == "table") then
		...
		for key, value in pairs(data) do
			tmp[index] = string.format(type(key) == "number" and "[%s]=%s" or "%s=%s", Serialize(key), type(value) == "string" and ('"' .. value .. '"') or Serialize(value));
			index = index + 1;
		end
...
local function Output(...)
	if(DEFAULT_CHAT_FRAME == nil or arg.n == 0) then
		return;
	end
